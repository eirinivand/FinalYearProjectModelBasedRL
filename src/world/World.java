...
public class World {
    ...
    void initWorld(int numOfSamplePolicies, int numOfParallelAgents, int runNumber, boolean runTD) throws Exception {
        ...
        try {
            ...
            Main.playClip(new File("resources\\done.wav"));
        } catch (Exception e) {
            e.printStackTrace();
        }
...
//                months + " months " +
    ...
    private void printReport() throws Exception {
        ...
        JFreeChart chart1 = ChartFactory.createXYLineChart("Average Utilities per Loop", "PI loop number", "Average Utility", averageUtilityXYSeries);
        ...
        JFreeChart chart4 = ChartFactory.createXYLineChart("Utilities of States and their progress per PI loop", "PI loop number", "Utility of State", averageUtilityXYSeriesStates);
       ...
       ByteArrayOutputStream bas4 = new ByteArrayOutputStream();
        ...
        g2.dispose();
        // DIAGRAM 3
        JFreeChart chart = ChartFactory.createXYLineChart("Policy Score per Loop", "PI loop number", "Policy Score", evaluationDataset);
        ...
        // DIAGRAM 4
        JFreeChart chart2 = ChartFactory.createXYLineChart("Visit Rates", "PI run number", "Visits", numbersDataset);
    ...
    private static PdfPTable createTable() throws DocumentException, IOException {
        ...
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < columns; j++) {
                ...
                Image image;
                if (state == null) {
                    image = Image.getInstance("resources\\rock.png");
                } else if (Environment.isEndState(state)) {
                    image = Image.getInstance("resources\\gem.png");
                } else {
                    image = Image.getInstance("resources\\upArrow.png");
                    if (state.getReward() != null && state.getReward() <= 0) {
                        image = Image.getInstance("resources\\upArrowRed.png");
                    }
